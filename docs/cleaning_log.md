...
| D2 | **Duplicate `delivery_id` with conflicting field values** — after exact-duplicate removal, 680 delivery IDs still appeared twice with differing `courier_id`/`city` | 1,360 rows (680 IDs) → 680 removed | Added a completeness score (count of non-null fields), sorted by `delivery_id` + completeness descending, kept the most complete record per ID | A single delivery_id must be unique for valid joins to incidents and for accurate counts. Where records conflicted, the more complete record was retained as the best available version. |
